qtconversion = {
	...
	'bool': 'toBoolean()',
	'string': 'toString()',
	'ControlPanel': 'toString()',
	'SpecialFolder': 'toString()'
} #'double':'Number()', - need to find right method for this

qtconversiontypecheck = {
	'int': 'isNumber()',
	'bool': 'isBoolean()',
	'string': 'isString()',
	'ControlPanel': 'isString()',
	'SpecialFolder': 'isString()'
}
qtconversiontypeconstant = {
	'int': 'LNZTYPE_int',
	'bool': 'LNZTYPE_bool',
	'string': 'LNZTYPE_string',
	'ControlPanel': 'LNZTYPE_controlpanel',
	'SpecialFolder': 'LNZTYPE_specialfolder'
}
cpptypes = {
	'int': 'int',
	'bool': 'bool',
	'string': 'QString',
	'ControlPanel': 'QString',
	'SpecialFolder': 'QString'
}
...
class ExposedFunctionArg(object):
	def __init__(self, arg, n, bOptional=False):
		if not bOptional:
			...
			if self.type not in qtconversion:
	...
	def isWindowArg(self):
		if self.type == 'string' and self.name == 'window':
			return True
		else:
			return False
	...
	def renderCheckCorrectType(self, strFnName):
		...
		return 'if (!ctx->argument(%d).%s)' % (
			self.n, method
		) + ' return g_ExceptionWrongTypeArg(ctx,FUNCTIONNAME,%d,%s);' % (self.n, qtconversiontypeconstant[self.type])

	def renderAssignVar(self, strFnName):
		if self.isWindowArg():
			return 'QScriptValue sv_val_window(ctx->argument(%d)); QString window(util_Au3WindowParseAndSetMode(sv_val_window)); QString strText(util_Au3WindowGetTextParamFromObj(sv_val_window));' % self.n
...
class ExposedFunction(object):
	...
	def buildCheckArguments(self):
		...
		if self.args == '':
			astrResult.append(
				'\t' + 'if (ctx->argumentCount()!=%d) return g_ExceptionWrongNumberArgs(ctx,FUNCTIONNAME,%d);' % (0, 0)
			)
		elif '=' not in self.args: #simple case, no optional arguments
			astrResult.append(
				'\t' + 'if (ctx->argumentCount()!=%d) return g_ExceptionWrongNumberArgs(ctx,FUNCTIONNAME,%d);' %
				(len(largs), len(largs))
			)
			...
			if len(Aargs) > 0:
				astrResult.append(
					'\t' + 'if (ctx->argumentCount()<%d) return g_ExceptionNotEnoughArgs(ctx,FUNCTIONNAME,%d);' %
					(len(Aargs), len(Aargs))
				)
			astrResult.append(
				'\t' + 'if (ctx->argumentCount()>%d) return g_ExceptionTooManyArgs(ctx,FUNCTIONNAME,%d);' %
				(len(AoptionalArgs) + len(Aargs), len(AoptionalArgs) + len(Aargs))
			)
		...
		if self.args != '' and not self.implementation.startswith('Javascript'):
			...
			for arg in self.args.split(','):
				...
				if argname not in allcode:
					raise Exception(
						'Error - Argument not used in function %s: type %s name %s' %
						(self.functionname, argtype, argname)
					)
			...
			for arg in self.args.split(','):
				...
				if bcurrent == 'mandatory' and bprev == 'opt':
					raise Exception(
						'Error - Optional arguments cannot be followed by mandatory args, in function %s , arg %s' %
	...
	for line in lines:
		...
		if line.startswith('///'):
			...
			if param == 'Function':
				...
				if data == 'true':
					currentFn.instanceMethod = True
				elif data == 'false':
					currentFn.instanceMethod = False
				else:
					raise Exception(
						'Function %s, instanceMethod parameter should be true or false, not %s' %
						(currentFn.functionname, data)
					)
			else:
		...
		elif line.rstrip() == '}' or line.strip(
		) == '{}': #note, don't cut at the close of a loop, only when there is no whitespace before it
