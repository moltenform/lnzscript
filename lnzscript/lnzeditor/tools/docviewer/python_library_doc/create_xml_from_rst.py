...
def unstructuretext(s):
    ...
    s = reComment.sub('\n', s)
    s = reVersion.sub('', s)
    return reRstDirective.sub('', s).replace('``', '').replace(
        '   .. note::', 'Note:'
    ) #remove :func: and so on. Remove ``literal`` marks
    ...
    for line in lines:
        ...
        if line.startswith('==='):
            ...
            if param == 'Section':
                ...
                if currentNamespace:
                    currentNamespace.consolidateDoc()
            ...
            if bInModDocumentation:
                raise Exception('not allowed- function inside module docs ' + line)
            bInFnDocumentation = True
            header, syntax = line.split('::')
            syntax = syntax.strip()
            if '.' in syntax and syntax.index('.') < (syntax.index('(') if '(' in syntax else 9999):
                syntax = syntax[syntax.index('.') + 1:]
            ...
            if currentFunction:
                currentFunction.consolidateDoc()
            ...
            if line.lstrip().startswith('.. function::'):
                currentFunction.static = True
            elif line.lstrip().startswith('.. method::'):
                currentFunction.static = False
        elif line.lstrip().startswith('.. class::') or line.lstrip().startswith('.. attribute::') or line.lstrip(
        ).startswith('.. data::'):
            raise Exception('You probably missed something here: ' + line)

        else:
            # keep the line break ??
            if line.endswith('.') or len(line) < 60 or (
    ...
    for section in arSections:
        ...
        for namespace in section.namespaces:
            ...
            for function in namespace.functions:
                ...
                fout.write(
                    '<function name="%s" fullsyntax="%s"  ' % (xmlescape(function.name), xmlescape(function.syntax))
                )
                print(function.name)
                if function.static:
                    fout.write(' >')
                else:
                    fout.write('instance="true">')
                if function.doc:
